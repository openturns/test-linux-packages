...
rm -rf /tmp/spectra-$pkgver
#git archive HEAD --prefix=$pkgname-$pkgver/ | bzip2 > /tmp/$pkgname-$pkgver.tar.bz2
cd /tmp

wget -c https://github.com/yixuan/spectra/archive/refs/tags/v${pkgver}.tar.gz
tar xzf v${pkgver}.tar.gz


rm -f "$pkgname"_$debver*
cp v${pkgver}.tar.gz "$pkgname"_$debver.orig.tar.gz
cd /tmp/spectra-$pkgver

cp -r ~/projects/openturns/spectra-pkg/debian .  
...
cp -v ~/projects/openturns/spectra-pkg/$pkgname.spec /tmp/v$pkgver.tar.gz $obs
