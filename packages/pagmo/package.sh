...
rm -rf /tmp/pagmo2-$pkgver
#git archive HEAD --prefix=$pkgname-$pkgver/ | bzip2 > /tmp/$pkgname-$pkgver.tar.bz2
cd /tmp

wget -c https://github.com/esa/pagmo2/archive/refs/tags/v${pkgver}.tar.gz
tar xzf v${pkgver}.tar.gz


rm -f "$pkgname"_$debver*
cp v${pkgver}.tar.gz "$pkgname"_$debver.orig.tar.gz
cd /tmp/pagmo2-$pkgver

cp -r ~/projects/openturns/pagmo-pkg/debian .  
...
cp -v ~/projects/openturns/pagmo-pkg/$pkgname.spec /tmp/v$pkgver.tar.gz $obs
cp -v ~/projects/openturns/pagmo-pkg/$pkgname-rpmlintrc $obs
